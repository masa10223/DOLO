...
def summarize_reinforce_learn_controls_mutants(control_matrix_files, mutant_matrix_files, contact_radius = 15, angle_threshold = 35, speed_threshold = 0.3, frame_interval = 5):
    ...
    mutants_results_dfs.columns = ['filename','id',
    ...
    control_results_dfs.columns = ['filename','id',
                     'non_contact_crawl','non_contact_turn','non_contact_pause',
                     'contact_crawl','contact_turn','contact_pause']
...
def plot_value_funcs_controls_mutants(control_results_dfs, mutants_results_dfs, filter = True):
    ...
    if filter:
        ...
        df_compare_melt = df_filtered.melt(id_vars="group", var_name="contact_type", value_name="value")

    else:
        df_compare['group'] = df_compare['group'].map({'control':'$white^{1118}$',
                                                        'mutant':'$orco^2$ , $Gr63a^1$',})
    ...
    plt.ylim([-1,1])
    plt.savefig('./Fig_paper/value_compare_boxplot.pdf')
    ...
    plt.yticks(fontsize = 20)
    plt.ylabel('')
    plt.savefig('./Fig_paper/value_compare_heatmap.pdf')
    ...
    plt.savefig('./Fig_paper/value_compare_mean_barplot_se.pdf')
...
if __name__ == "__main__":
    ...
    control_results_dfs, mutants_results_dfs = summarize_reinforce_learn_controls_mutants(control_matrix_files, mutant_matrix_files,
                                                                                          contact_radius = contact_radius, 
                                                                                          angle_threshold = angle_threshold, 
                                                                                          speed_threshold = speed_threshold, 
                                                                                          frame_interval = frame_interval)
